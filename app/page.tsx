...
export default async function HomePage() {
  const [categories, popularFamilies, dbBuyers] = await Promise.all([
    ...
    getPopularFamilies(12),
    getBuyers(),
  ])

  const popularDevices = popularFamilies.map(dbFamilyToDevice)
  const buyers = dbBuyers.map(dbBuyerToBuyer)

  // Real counts from DB
  const totalBuyers = dbBuyers.length
  const totalDevices = popularFamilies.length > 0 ? `${94}+` : `${platformStats.totalDevices}+`
    ...
    <div className="min-h-screen flex flex-col">
      ...
      >
        ...
        <div
          ...
          style={{
            backgroundImage:
              'linear-gradient(#00C4B4 1px, transparent 1px), linear-gradient(90deg, #00C4B4 1px, transparent 1px)',
        ...
        <div className="relative max-w-4xl mx-auto text-center">
          ...
          <p className="text-lg sm:text-xl text-slate-300 max-w-2xl mx-auto mb-10 leading-relaxed">
            Compare offers from{' '}
            <strong className="text-white">{totalBuyers}+ verified buyers</strong> and get the best
            price for your used phone, tablet, or laptop.{' '}
            <span className="text-teal-300">Free. Instant. No pressure.</span>
          </p>

          {/* Search */}
          <div className="max-w-2xl mx-auto mb-8">
            <DeviceSearch placeholder="Search any phone, tablet, laptop..." large={true} />
          </div>

          {/* Popular searches */}
          <div className="flex flex-wrap items-center justify-center gap-2 text-sm">
            <span className="text-slate-500">Popular:</span>
            {['iPhone 16 Pro', 'Galaxy S25', 'Pixel 9 Pro', 'MacBook Pro', 'iPad Pro'].map(
              item => (
                <button
                  key={item}
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 divide-x divide-slate-100">
            {[
              ...
              },
              {
                ...
                value: `${(platformStats.totalSellers / 1000).toFixed(0)}K+`,
                icon: <Users className="w-4 h-4" />,
              },
              {
                label: 'CO₂ Saved (tons)',
                value: `${platformStats.co2SavedTons.toLocaleString()}`,
                icon: <Leaf className="w-4 h-4" />,
              },
            ].map(stat => (
              <div
                key={stat.label}
                className="flex flex-col items-center justify-center py-6 px-4 gap-2 text-center"
              >
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 mb-14">
            {[
              ...
              },
              {
                ...
                desc: 'Offers update regularly. No stale prices. What you see is what you get.',
              },
              {
              ...
              <div
                key={i}
                className="flex gap-4 p-5 rounded-2xl border border-slate-100 hover:border-teal-200 hover:bg-teal-50/30 transition-all"
              >
      ...
      <section className="py-16 px-4 bg-gradient-to-br from-emerald-900 to-teal-900">
        <div className="max-w-4xl mx-auto text-center">
          ...
          <p className="text-emerald-200 text-lg mb-6 max-w-2xl mx-auto leading-relaxed">
            Manufacturing one smartphone produces ~57kg of CO₂. When you sell instead of trash, you
            extend that device&apos;s life and help keep toxic materials out of landfills.
          </p>
          <Link
            href="/sustainability"
