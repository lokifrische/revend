...
const paymentIcons: Record<string, string> = {
  PayPal: '🅿️', Venmo: '💜', Zelle: '💙', 'Cash App': '💚', Check: '📄', 'Amazon Gift Card': '🛒',
}
      ...
      <main className="flex-1 pt-20">
        ...
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {buyers.map(buyer => (
              <div key={buyer.id} className="bg-white rounded-2xl border border-slate-200 overflow-hidden hover:shadow-lg transition-all">
                {/* Header */}
                ...
                <div className="grid grid-cols-3 divide-x divide-slate-100 border-b border-slate-100">
                  <div className="px-4 py-3 text-center">
                    ...
                    <p className="text-sm font-bold text-navy-800">{buyer.yearsActive}</p>
                  </div>
                  <div className="px-4 py-3 text-center">
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-1.5 flex-wrap">
                      {buyer.paymentMethods.map(m => (
                        <span key={m} className="flex items-center gap-1 px-2 py-1 bg-slate-100 rounded-lg text-xs text-slate-600">
                          {paymentIcons[m] ?? '💳'} {m}
                        </span>
                      ))}
                    </div>
            ...
            <div className="bg-gradient-to-br from-navy-800 to-navy-700 rounded-2xl border border-navy-600 p-6 flex flex-col items-center justify-center text-center text-white">
              ...
              <p className="text-slate-400 text-sm mb-5 leading-relaxed">
                Join Revend as a buyer partner and get qualified, ready-to-sell leads delivered daily.
              </p>
              <Link
                href="/partners/apply"
